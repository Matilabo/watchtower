...
Files here are referenced from stylesheets or TypeScript and are emitted by the
bundler with a content hash. That matters for two reasons: the URL in the built
CSS is rewritten relative to the stylesheet, so it resolves under any base path
(a project site on GitHub Pages is served from `/<repo>/`), and the hash means
a changed image is never served from a stale cache.

## The backdrop

Two derivatives of one photograph, both committed:

| File | Size | Used by |
| --- | --- | --- |
| `background.jpg` | 2560x1440, ~152 kB | screens wider than 900px |
| `background-1280.jpg` | 1280x800, ~48 kB | everything narrower |
...
node -e "const s=require('sharp');for(const v of [{o:'src/assets/background.jpg',w:2560,h:1440,q:70},{o:'src/assets/background-1280.jpg',w:1280,h:800,q:68}])s('assets-src/background-original.jpg').resize(v.w,v.h,{fit:'cover'}).modulate({brightness:0.88,saturation:0.92}).jpeg({quality:v.q,progressive:true,mozjpeg:true}).toFile(v.o)"
npm uninstall sharp
```
